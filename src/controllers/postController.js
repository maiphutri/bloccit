...
module.exports = {
  ...
  destroy(req, res, next) {
    postQueries.deletePost(req, (err, authorized, post) => {
      if (err) {
    ...
    postQueries.getPost(req.params.id, (err, post) => {
      if (err || post == null) {
        ...
        if (authorized) {
          ...
          req.flash("notice", "You are not authorized to do that.")
          res.redirect(`/topics/${post.topicId}/posts/${req.params.id}`)
        }
      }
    })
  },
  ...
  update(req, res, next) {
    postQueries.updatePost(req, req.body, (err, post) => {
      if (err || post == null) {
        res.redirect(401, `/topics/${req.params.topicId}/posts/${post.id}`)
      } else {
